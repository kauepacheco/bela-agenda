...
import { BookingError, changeBooking, createBooking, appointmentStatuses } from "@/lib/booking-service";
import { phoneSchema } from "@/lib/booking-policy";
import { NextResponse } from "next/server";
import { z } from "zod";

const appointmentSchema = z.object({
  clientId: z.string().optional(), clientName: z.string().trim().min(2).max(120).optional(),
...
export async function PATCH(request: Request) {
  const context = await getCurrentContext();
  if (!context) return NextResponse.json({ error: "Não autorizado." }, { status: 401 });
  const parsed = z.object({ id: z.string().min(1), status: z.enum(appointmentStatuses).optional(), startsAt: z.iso.datetime({ offset: true }).transform((value) => new Date(value)).optional() })
