...
export function PublicBooking({ business, services, enabled }: { business: { name: string; city: string; address: string | null; slug: string; phone: string | null }; services: Service[]; enabled: boolean }) {
  ...
  return <main className="booking-page"><header className="booking-top"><div className="booking-brand"><span><Sparkles size={17}/></span>Santa Agenda</div><span>Seu tempo de cuidado começa aqui</span></header><div className="booking-layout"><aside className="booking-business"><div className="business-logo">{business.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</div><span className="eyebrow">AGENDE ONLINE</span><h1>{business.name}</h1><p><MapPin size={16}/>{[business.address, business.city].filter(Boolean).join(", ")}</p><div className="booking-promise"><Sparkles size={17}/><div><strong>Um momento para você</strong><span>Escolha seu serviço, encontre um horário e deixe o resto com a equipe.</span></div></div><p className="timezone-note">Todos os horários são de Brasília.</p></aside><section className="booking-form">
  ...
  </section></div><footer className="booking-footer">Feito para cuidar do seu tempo · <strong>Santa Agenda</strong></footer></main>;
}
