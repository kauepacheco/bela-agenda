...
type Service = { id: string; name: string; durationMin: number; priceCents: number; professionals: { professional: Professional }[] };
...
export function PublicBooking({ business, services, enabled }: { business: { name: string; city: string; address: string | null; slug: string; phone: string | null }; services: Service[]; enabled: boolean }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/public/${business.slug}/appointments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serviceId, professionalId, startsAt: time, clientName: data.get("name"), clientPhone: data.get("phone") }) });
